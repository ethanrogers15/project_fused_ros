...
def get_current_suffix() -> str:
    # Use current timestamp as a unique suffix
...
def generate_launch_description():
    ...
    lepton_param_file = join(usb_cam_path, 'config', 'lepton_params_estimate.yaml') #NOTE: change for different parameters if needed
    webcam_param_file = join(usb_cam_path, 'config', 'webcam_params_checker.yaml') #NOTE: change for different parameters if needed

    # Declare base node name launch argument
    base_node_name = LaunchConfiguration('base_node_name')

    # Generate the current unique suffix
    ...
    lidar_info_publisher = Node(
        ...
        name=generate_unique_name(base_node_name.perform(), current_suffix),
        output='screen',
        remappings=[
            ('/camera_info_placeholder', '/flexx2_camera_node/camera_info_calibrated')
        ],
        parameters=[{
    ...
    image_proc_lidar_node = Node(
        ...
        remappings=[
            ...
            # ('/camera_info', '/flexx2_camera_node/camera_info_calibrated'), #NOTE: change for different parameters if needed
    ...
    testing_node = Node(
        ...
        remappings=[
            ...
            ('/lidar_calib_info_placeholder', '/flexx2_camera_node/camera_info_calibrated'),
        ...
        parameters=[{
            ...
            'no_extrinsic_rotations': True, #NOTE: change for different parameters if needed
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            'base_node_name',
            default_value='my_node',  # Default base name for nodes
            description='Base name for all nodes'
        ),
        lidar_info_publisher,
        pmd_container,
        lepton_node,
        webcam_node,
        image_proc_lidar_node,
        image_proc_thermal_node,
        image_proc_webcam_node,
        testing_node,
    ])
