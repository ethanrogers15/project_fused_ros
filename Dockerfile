...
RUN apt-get update && sudo rosdep fix-permissions && rosdep update && rosdep install --from-paths /ros2_object_detection/src --ignore-src -r -y

# Clean up
RUN apt-get clean && rm -rf /var/lib/apt/lists/*

# Set entrypoint script
ENTRYPOINT ["/ros2_object_detection/ros_ws_entrypoint.sh"]
...
RUN echo "source /opt/ros/humble/setup.bash" >> ~/.bashrc \
    && echo "[[ -f /ros2_object_detection/install/setup.bash ]] && source /ros2_object_detection/install/setup.bash" >> ~/.bashrc
